...
static void FillFactoryFrontierTrainerParty(u16 trainerId, u8 firstMonId)
{
    ...
    if (trainerId < FRONTIER_TRAINERS_COUNT)
    {
        ...
        u8 battleMode = VarGet(VAR_FRONTIER_BATTLE_MODE);
        // u8 challengeNum = gSaveBlock2Ptr->frontier.towerWinStreaks[battleMode][FRONTIER_LVL_50] / 7; FIXED BUG: Factory was using the Tower win streak to calculate IVs
        u8 challengeNum = gSaveBlock2Ptr->frontier.factoryWinStreaks[battleMode][FRONTIER_LVL_50] / 7;
